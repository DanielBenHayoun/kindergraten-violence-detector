...
def finetune(modelPath,numEpochs, lr, stepSize, decayRate, trainBatchSize, seqLen, memSize,
		...
		if (epoch+1) % evalInterval == 0:
			...
			for j, (inputs, targets) in enumerate(testLoader):
				...
				if evalMode == 'centerCrop':
					...
					inputVariable1 = Variable(inputs.permute(1, 0, 2, 3, 4), volatile=True)
				else:
...
if __name__ == '__main__':
	...
	parser.add_argument('--modelPath', type=str, default='./experiments_violence/bestModel.pth', help='Number of epochs')
	...
	finetune(modelPath,numEpochs, lr, stepSize, decayRate, trainBatchSize, seqLen, memSize,
			 evalInterval, evalMode, numWorkers, outDir, fightsDir_train,
			 noFightsDir_train, fightsDir_test, noFightsDir_test)
