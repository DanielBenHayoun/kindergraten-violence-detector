...
class ImageReader(object):
    ...
    def __next__(self):
        ...
        frame_name=self.file_names[self.idx]
        self.idx = self.idx + 1
...
def run_demo(net, image_provider, height_size, cpu, track, smooth,target_dir=''):
    net = net.eval()
    if not cpu:
        net = net.cuda()
    ...
    for img,frame_name in image_provider:
        ...
        for i,pose in zip(range(len(current_poses)),current_poses):
            ...
            blank = create_blank()
            pose.draw(blank)
        print("saving: " +os.path.join(target_dir,os.path.basename(frame_name)))   
        res=cv2.imwrite(os.path.join(target_dir,os.path.basename(frame_name)), img)
...
def create_blank(width=300, height=300,rgb_color=(0, 0, 0)):
